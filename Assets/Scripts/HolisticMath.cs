...
public class HolisticMath
{
    ...
    {
        //stops an error when retruning a (0,0,0) vector for division (division by 0 error)
        if(HolisticMath.Distance(new Coords(0,0,0), vector) <= 0)
        {
            return position;
        }
        //find the angle between two vectors (the vector that we want to move in and our current facing vector for the player)
        //and rotate
        float angle = HolisticMath.Angle(vector, facing);
        //the up coords of the world not the tank i.e. (0,1,0)!
        ...
        //add the current vector position to the new vecotor for translation:
    ...
    {
        //calculate vector from B to A which is creating a line from A to B:

        t = Mathf.Clamp(t, 0, 1);

        Coords v = new Coords(B.x - A.x, B.y - A.y, B.z - A.z);
        //create a point on the line from A -> B:
        float xt = A.x + v.x * t;
        float yt = A.y + v.y * t;
        float zt = A.z + v.z * t;
